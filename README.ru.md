...
```mermaid
graph TD
    A["Старт: DialogYAMLBuilder.build()"] --> B{"1. Чтение и парсинг YAML"};
    B -- Необработанные данные --> C{"2. Генерация состояний FSM"};
    B -- Необработанные данные --> D{"3. Создание моделей Pydantic"};
    D -- Имена функций --> E{"4. Связывание зарегистрированных функций"};
    C & E --> F{"5. Сборка объектов aiogram-dialog"};
    F --> G{"6. Регистрация диалогов в роутере"};
    G --> H["Конец: Возврат настроенного роутера"];
```
...
Эта диаграмма иллюстрирует, как взаимодействуют основные внутренние компоненты `dialog-yaml`.

```mermaid
graph LR
    subgraph `Ваш код на Python`
        direction TB
        App(Приложение)
        Funcs(Пользовательские функции)
    end

    subgraph `Библиотека dialog-yaml`
        direction TB
    ...
    subgraph `Данные и объекты`
        direction TB
        YAML(YAML-файлы)
        PydanticMs(Модели Pydantic)
        AD["Объекты aiogram-dialog"]
    end

    App -- "вызывает" --> Builder
    Funcs -- "регистрируются в" --> FuncReg

    Builder -- "использует" --> Reader
    Builder -- "использует" --> States
    Builder -- "использует" --> Factory

    Reader -- "читает" --> YAML
    
    Factory -- "создает" --> PydanticMs
    PydanticMs -- "ссылаются на" --> FuncReg

    Builder -- "преобразует" --> PydanticMs
    Builder -- "создает" --> AD
```
...
Хотя YAML отлично подходит для определения структуры UI, для бизнес-логики вам понадобятся функции на Python. `dialog-yaml` позволяет связывать функции Python с вашими виджетами для двух основных целей: **обработчики событий** и **геттеры данных**.
...
`dialog-yaml` использует глобальный синглтон для реестра. Вам не нужно передавать его в процессе сборки. Самое важное — **убедиться, что модуль Python, в котором вы определяете и регистрируете свои функции, импортирован** до запуска бота.
...
Смотрите директорию [examples/mega](examples/mega/) для комплексного примера, который демонстрирует:
