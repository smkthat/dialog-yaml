"""The `core.models` module provides the `YAMLModelFactory` class
...
class YAMLModelFactory:
    ...
    def is_valid_model_class(
        ...
        if not model_class or not issubclass(
            model_class, (YAMLModel, BaseModel)
        ):
    ...
    def set_classes(cls, models_classes: Dict[str, Type[YAMLModel]]) -> None:
        ...
        if not isinstance(models_classes, dict):
            raise DialogYamlException(
                "models_classes must be a dictionary "
                "of strings to YAMLModel classes"
            )
    ...
    def create_model(cls, yaml_data: Dict[str, Any]) -> YAMLModel:
        ...
        if not yaml_data or not isinstance(yaml_data, dict):
            raise DialogYamlException(
                "yaml_data must be a non-empty dictionary"
            )
