...
from core.models.widgets.scrolls import (
    ScrollingTextModel, StubScrollModel, NumberedPagerModel,
    FirstPageModel, PrevPageModel, CurrentPageModel, NextPageModel, LastPageModel
)
...
class TestScroll(TestWidgetBase):
    @pytest.mark.parametrize('input_data, expected_model_cls, expected_widget_cls', [
        ({'numbered_pager': 'scroll_no_pager'},
         NumberedPagerModel, NumberedPager),
        ({'numbered_pager': {
        ...
        ({'scrolling_text': {
          ...
          'id': 'text_scroll',
          'page_size': 1000}},
         ScrollingTextModel, ScrollingText),
        ({'stub_scroll': {'id': 'stub_scroll', 'pages': 'pages'}},
         StubScrollModel, StubScroll),
        ({'stub_scroll': {'id': 'stub_scroll', 'pages': 'pages'}},
         StubScrollModel, StubScroll),
        ({'first_page': 'scroll_no_pager'},
         FirstPageModel, SwitchPage),
        ({'next_page': 'scroll_no_pager'},
         NextPageModel, SwitchPage),
        ({'current_page': 'scroll_no_pager'},
         CurrentPageModel, SwitchPage),
        ({'prev_page': 'scroll_no_pager'},
         PrevPageModel, SwitchPage),
        ({'last_page': 'scroll_no_pager'},
